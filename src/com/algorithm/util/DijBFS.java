...
public class DijBFS {
	...
	private static int selectNum = 500;
	...
	private static Element[][] costTable; // D^r(p,q)表
	...
	public static void initialize() {
		...
		for (int i = 0; i < condLength + 1; i++) {
			for (int k = 0; k < condLength + 2; k++) {
				if (k != i) {
					int[][] newMatrix = new int[veterNUM][veterNUM];
					for (int g = 0; g < veterNUM; g++) {// 拷贝矩阵
						for (int h = 0; h < veterNUM; h++) {
							newMatrix[g][h] = connMatrix[g][h];
						}
					}
					for (int j = 0; j < condLength + 2; j++) {
						if (i != j && k != j) {
							int zeropoint = allDemand.get(j);// 扫描zeropoint这一列，全都置0
							for (int p = 0; p < veterNUM; p++) {
								newMatrix[p][zeropoint] = 0;
							}
						}
					}
					temp = Dijkstra.dijkstra(newMatrix, allDemand.get(i),
							allDemand.get(k));
					if (Dijkstra.path != null) {
						costPath0 = new Element(temp, Dijkstra.path);
						costTable[i][k] = costPath0;
					} else {
						temp = 0;
		...
		// 所有到起点的路径置0
		for (int j = 0; j < condLength + 1; j++) {
			costTable[j][0].setKey(0);
			costTable[j][0].setValue(null);
		}
		// 起点直接到终点的列置零
		costTable[0][condLength + 1].setKey(0);
		costTable[0][condLength + 1].setValue(null);
	}
	...
	private static boolean containRepeatPoints(ArrayList<Integer> tempPath,
	...
	private static void broadFirstSearch() {
		...
		path.add(source);// 访问结点startPoint
		...
		while (savedPath.size() != 0) {
			for (int i = 0, loop = savedPath.size(); i < loop; i++) {
				...
				if (currentPointList != null && currentPointList.size() != 0) {
					for (int j = 0, loop2 = currentPointList.size(); j < loop2; j++) {// 进行下一次搜索
						...
						ArrayList<Integer> tempPath = new ArrayList<Integer>(
								path1);
						if (containRepeatPoints(tempPath, cuurentPoint,
								neighborpoint)) {// 判断有无重点
							continue;
						} else if (neighborpoint == target) {
							if (tempPath.size() == allDemand.size() - 1) {
								tempPath.add(neighborpoint);
								ArrayList<Integer> sucPath = getCompletePath(tempPath);
								if (sucPath.containsAll(condList)) {// 判断包含所有必经点
									minCost = pathCost(tempPath);
									// if (minCost < successedPathCost) {
									// successedPath = sucPath;
									// successedPathCost = minCost;
									// }
